...
# The version control channel row information. This starts off incomplete and
# gets filled in during the setup phase of this programme's execution.
...
Where options are:
       -c     turns on the control channel. This puts a version identifying row into 
              the output file at row zero and the row after the last channel row.
       -f n   ignore the first n rows of the source file. Useful for skipping headers.
              The range for n is 0 through 9 inclusive.
...
# ------------------------------------------------------------------------------
def is_tone_not_set(tone_str):
    """True if there is no tone in tone_str
    """
    # Make sure the tone string is really there. If it isn't, then report this
    # back to the caller. The input can have no values in this field.
    if tone_str is None:
        logging.debug("Tone has a null value. Assuming it is not set.")
        return True

    if len(tone_str.strip()) == 0:
        logging.debug("The tone string was zero length. Assuming it is not set.")
        return True
...
def is_ctcss_tone(tone_str):
    ...
    if CTCSS_TONE_REGEX.match(raw_tone) is None:
        logging.debug(f"No CTCSS tone was found.")
        return (TONE_TYPE_NO_TONE, tone_str)
    ...
    try:
        ...
        logging.debug(f"Not a CTCSS tone: {raw_tone}.")
        return (TONE_TYPE_UNRECOGNISED, tone_str)

    if tone_value in STANDARD_CTCSS_TONES:
        # A valid CTCSS tone was found.
        logging.info(f'Registered a valid CTCSS tone: {tone_value}Hz.')
        return (TONE_TYPE_CTCSS, str(tone_value))

    elif tone_value in EXTENDED_CTCSS_TONES:
        logging.debug(f"Extended CTCSS tone encountered: {tone_value}Hz.")
        return (TONE_TYPE_CTCSS, str(tone_value))
    else:
        logging.debug("Looks like a CTCSS tone but does not match the known "
...
def is_dcs_tone(tone_str):
    ...
    Returns a tuple. The first element tells you if the input is a DCS tone or
    not. These are indicated by TONE_TYPE_DCS or TONE_TYPE_UNRECOGNIED
    respectively. The second element is the tone in string form ([0-9][1-9]).
    """
    raw_tone = tone_str.strip()
    # Check the generalised pattern for a DCS tone
    if DCS_TONE_REGEX.match(raw_tone) is None: 
        logging.debug(f"This value is not a valid DCS tone ({raw_tone}).")
        return (TONE_TYPE_UNRECOGNISED, tone_str)
    ...
    if tone_str_len == 4:
        # DCS tones often start with a capital D. They are not supposed to start
        # with a lower case d, but just give the benefit of the doubt.
        if raw_tone[0] in ('D', 'd') and raw_tone[1:] in DCS_TONES:
            logging.debug(f"Got a valid 4-character DCS tone: {raw_tone}")
    ...
    # Perhaps the D wasn't prepended - check to see if digits are present
        ...
        if raw_tone in DCS_TONES:
            logging.debug(f"Got a valid 3-character DCS tone: {raw_tone}")
            return (TONE_TYPE_DCS, raw_tone)
        else:
            logging.debug(f"This value is not a valid DCS tone: {raw_tone}")
            return (TONE_TYPE_UNRECOGNISED, tone_str)
        ...
        if '0' + raw_tone in DCS_TONES:
            logging.debug(f"Got a valid 2-character DCS tone: {raw_tone}")
            return (TONE_TYPE_DCS, raw_tone)
        else:
            logging.debug(f"This value is not a valid DCS tone: {raw_tone}")
            return (TONE_TYPE_UNRECOGNISED, tone_str)
...
def parse_tone(tone_str):
    ...
    The tone could a CTCSS tone, a DCS tone, no tone which is okay or
    unrecognisable as a tone value which is not.
    
    If a CTCSS tone is spotted, this is flagged as TONE_TYPE_CTCSS.
    If it's DCS, then TONE_TYPE_DCS 
    If it's blank, then this is flagged as TONE_TYPE_NO_TONE.
    Otherwise, this is an error condition and the type is set as TONE_TYPE_UNRECOGNISED
    """
    # First - check to see if there is no tone set. If no type of tone was
    # detected. This is okay, because many channels will not have a tone of any
    # description.
    if is_tone_not_set(tone_str):
        return (TONE_TYPE_NO_TONE, None)

    # Looks like we might have a tone, check for a CTCSS style tone
    ...
    # The tone is not valid. It is neither a recognised type nor blank.
    logging.error(f'A tone value is set, but not recognised as valid CTCSS nor DCS: >>>{tone_str}<<<')
...
def parse_tones(tx_tone_str, rx_tone_str):
    ...
    (parsed_tx_tone_type, parsed_tx_tone_value) = parse_tone(tx_tone_str)
    (parsed_rx_tone_type, parsed_rx_tone_value) = parse_tone(rx_tone_str)

    if parsed_tx_tone_type == TONE_TYPE_NO_TONE:
        # This channel does not have a tone set. That's okay.
        logging.info("Tone has not been set for this channel.")
    
    elif parsed_tx_tone_type == TONE_TYPE_CTCSS:
        # The local station has to transmit a tone to access the remote
        # station. We need to check to see if an Rx tone has been set too.
        tx_ctcss_tone = parsed_tx_tone_value
        if parsed_rx_tone_type == TONE_TYPE_CTCSS:
            ...
            rx_ctcss_tone = parsed_rx_tone_value
        else:
            tone_mode = BASIC_TONE_MODE

    elif parsed_tx_tone_type == TONE_TYPE_DCS:
        tx_dcs_tone = parsed_tx_tone_value
        tone_mode = DCS_TONE_MODE
        if parsed_rx_tone_type == TONE_TYPE_DCS:
            rx_dcs_tone = parsed_rx_tone_value
...
def parse_frequency(raw_frequency):
    ...
    if isinstance(raw_frequency, str):
        ...
        try:
            ...
            logging.warning(f"Unrecognisable frequency value: {ics_row[2]} for {ics_row[1]}.")
...
def ics_parse(raw_ics_data):
    ...
    for line in raw_ics_data:
        ...
        if is_null_row(ics_row[1], ics_row[2]):
            ...
            logging.warning(f"A null row was encountered at row: {ics_row_counter}")
            ics_row_counter += 1
            continue
    ...
    with open(VERSION_TOKEN_FILE, newline='') as version_file:
        for line in version_file:
            if blank_line_re.match(line):
                ...
                line.strip()
                logging.error(f"Unexpected content in the version token file: {line}")
                sys.exit(11)
...
def process_options():
    ...
    try:
        ...
        opts, args = getopt.getopt(sys.argv[1:], 'f:ct:', 
                                    ['skip-first-rows=', 'control_ch', 
                                    'tone='])
    except getopt.GetoptError as err:
        print(f'Invalid command line option: {err}')
        logging.error(f'Invalid command line option: {err}')
        usage()
        sys.exit(2)
    ...
    for o, a in opts:
        if o in ('-f', '--skip-first-rows'):
            ...
            if tone_type == TONE_TYPE_CTCSS:
                ...
                logging.info(f'Default CTCSS tone has been set to: {g_default_ctcss_tone}Hz')
        else:
...
# ------------------------------------------------------------------------------
def gen_logfile_name():
    """
    Generates a file name that has the following format:
    ...
    For example, if the executable name is 'i2c.py':
    i2c20230304-172000.log would be the file name.
    """
    dt = datetime.datetime.now()
    fname = (sys.argv[0]).split('.')[0] +\
        f'{dt.year}{dt.month:02d}{dt.day:02d}-' +\
        f'{dt.hour:02d}{dt.minute:02d}{dt.second:02d}.log'
    return fname

# ------------------------------------------------------------------------------
def start_logging():
    logging.basicConfig(filename=gen_logfile_name(),
...
if __name__ == '__main__':
    ...
    try:
        ...
        logging.error('Wrong number of files specified on the command line.')
        sys.exit(-1)
